...
def extract_data(raw_text):
    ...
    "B_W_G_Received": r"Food\s+received.*?\n(?:.*?Rice[^\d]*\d+kgs.*?\n)*?.*?(?:Beans|GreenGrams|Green grams|GG|Western|G.G)[^\d]*(\d+)",

    # Excess food section
    "Rice_Excess": r"Excess.*?\n.*?Rice[^\d]*(\d+\.?\d*)",
    "B_W_G_Excess": r"Excess.*?\n(?:.*?Rice[^\d]*\d+kgs.*?\n)*?.*?(?:Beans|GreenGrams|Western|G.G|GG)[^\d]*(\d+)",

    # Food remaining section
    "Rice_Remaining": r"Food\s+remaining.*?\n.*?Rice[^\d]*(\d+\.?\d*)",
    "B_W_G_Remaining": r"Food\s+remaining.*?\n(?:.*?Rice[^\d]*\d+kgs.*?\n)*?.*?(?:Beans|GreenGrams|Green grams|Western|G.G|Wairimu)[^\d]*(\d+)",

    # Temperatures

    "B_W_G_Temperatures": r"Average\s+Temp\s+for\s+(?:[Bb]eans|Green\s?[Gg]rams?|Western|G\.G|GG)\s*[:=]?\s*?(\d+\.?\d*)\s*[\u00B0\u2070]?[Cc]",
    "Rice_Temp": r"Average\s+Temp\s+for\s+(?:[Ww]hite|[Tt]umeric)?\s*[Rr]ice\s*[:=]?\s*?(\d+\.?\d*)\s*[\u00B0\u2070]?[Cc]",
    ...
    "Start_Taping": r"Started\s+tapping\s+at\s*:?\s*(\d{1,2}:?\d{2}\s?(?:[apAP][mM]|hrs|HRS|Hrs)?)",
    "Stop_Taping": r"Started\s+tapping\s+at\s*:?\s*\d{1,2}:?\d{2}\s?(?:[apAP][mM]|hrs|HRS|Hrs)?\s+to\s*:?\s*(\d{1,2}:?\d{2}\s?(?:[apAP][mM]|hrs|HRS|Hrs)?)",

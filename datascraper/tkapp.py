...
def extract_data(raw_text):
    data_pattern = {
        ...
        "Rice Received": r"Rice ?: ?(\d+)(?:kgs|Kgs|kg|KG)",
        "Western Received": r"Western ?: ?(\d+)(?:kgs|Kgs|kg|KG)",
        "Rice Excess": r"Rice ?: ?(\d+)(?:kgs|Kgs|kg|KG)",
        "Western Excess": r"Western ?: ?(\d+)(?:kgs|Kgs|kg|KG)",
        "Food Arrival": r"Food arrival ?: ?(.*?)\n",
        "Start Temp Check": r"Started Temp Check at ?: ?(.*?) to",
        "End Temp Check": r"to (.*?)\n",
        "Rice Temp": r"Average Temp for Rice ?: ?(\d+\.?\d*)°[Cc]",
        "Western Temp": r"Average Temp for Western ?: ?(\d+\.?\d*)°[Cc]",
        "Rice Remaining": r"Rice ?: ?(\d+)(?:kgs|Kgs|kg|KG)",
        "Western Remaining": r"Western ?: ?(\d+)(?:kgs|Kgs|kg|KG)",
        "Missed Food": r"No of kids missed food ?: ?(\d+)",
        "Start Serving": r"Started tapping at ?: ?(.*?) to",
        "Stop Serving": r"to (.*?)\n",
...
app.layout = html.Div([
    ...
    html.Div([
        dash_table.DataTable(
            id='data-table',
            columns=[
                {"name": col, "id": col, "editable": True} for col in [
                    "Kitchen", "School", "Date", "Menu", "Projected Kids", "Actual Meals Served", "Successful Taps",
                    "F4E Supported Kids", "Teachers", "Staff Meals", "Failed Taps", "New Registrations",
                    "New Tags Replaced", "Rice Received", "Western Received", "Rice Excess", "Western Excess",
                    "Food Arrival", "Start Temp Check", "End Temp Check", "Rice Temp", "Western Temp",
                    "Rice Remaining", "Western Remaining", "Missed Food", "Start Serving", "Stop Serving",
